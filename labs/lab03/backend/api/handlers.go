...
func (h *Handler) SetupRoutes() *mux.Router {
	...
	apiRouter.HandleFunc("/cat/{code}", h.GetHTTPStatusImage).Methods("GET")
...
var transparentPNG = []byte{
	...
	0x03, 0x03, 0x01, 0xff, 0xa5, 0x93, 0x3f, 0xe7, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4e, 0x44,
	0xae, 0x42, 0x60, 0x82,
}

// GetHTTPStatusImage handles GET /api/cat/{code}
func (h *Handler) GetHTTPStatusImage(w http.ResponseWriter, r *http.Request) {
	codeURL := mux.Vars(r)["code"]
	code, err := strconv.Atoi(codeURL)
	if err != nil || code < 100 || code > 599 {
		h.writeError(w, 400, "Invalid request data")
		return
	}
	...
	w.WriteHeader(200)
	_, _ = io.Copy(w, bytes.NewReader(transparentPNG))
}
